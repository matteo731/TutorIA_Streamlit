...
# ── Régua absoluta de Nível por cluster de loja (v14) ───────────────────────
# Cortes: índice = consumo_real / consumo_previsto. Menor = melhor.
# Fronteira ≤1.00 é sempre "bom" (B ou superior). Cortes fixos, estabilidade
# validada nos 7 meses disponíveis (jan-jul/2026). Ordem: melhor → pior.
REGUA_CLUSTER = {
    # cluster_key           edges                          labels
    ("BK",  "RUA"):        ([0, 0.80, 1.00, 1.22, 1.44],  ["A",  "B",  "C",  "D" ]),
    ("BK",  "EMPREENDIMENTO"): ([0, 0.57, 0.79, 1.00, 1.16, 1.32, 1.48],
                                                           ["A+", "A",  "B",  "C",  "D",  "E"]),
    ("BK",  "KSK"):        ([0, 0.79, 1.00, 1.20, 1.39],  ["A",  "B",  "C",  "D" ]),
    ("PLK", "EMPREENDIMENTO"): ([0, 0.60, 0.80, 1.00, 1.15, 1.30],
                                                           ["A+", "A",  "B",  "C",  "D" ]),
    ("SBUX","EMPREENDIMENTO"): ([0, 0.59, 0.79, 1.00, 1.15, 1.30],
                                                           ["A+", "A",  "B",  "C",  "D" ]),
    ("SBUX","RUA"):        ([0, 0.77, 1.00, 1.15, 1.30],  ["A",  "B",  "C",  "D" ]),
    ("SBUX","AEROPORTO"):  ([0, 0.49, 0.75, 1.00, 1.15, 1.30],
                                                           ["A+", "A",  "B+", "C",  "D" ]),
    ("SBUX","QUIOSQUE"):   ([0, 0.75, 1.00, 1.50],        ["A",  "B",  "C"        ]),
}
# Régua absoluta de Nível para grupos (Coordenador / Gerente Regional)
# Índice de grupo = média dos índices das lojas da carteira. Âncora em 1.00.
REGUA_GRUPO = {
    "BK":   ([0, 0.90, 1.00, 1.10, 99], ["A", "B", "C", "D"]),
    "PLK":  ([0, 0.90, 1.00, 99],        ["A", "B", "C"      ]),
    "SBUX": ([0, 0.90, 1.00, 99],        ["A", "B", "C"      ]),
}

def _nivel_absoluto(indice, edges, labels):
    """Atribui nível absoluto a um escalar ou Series usando cortes fixos."""
    import pandas as _pd
    if isinstance(indice, _pd.Series):
        return _pd.cut(indice, bins=edges, labels=labels, include_lowest=True)
    for i, (lo, hi) in enumerate(zip(edges[:-1], edges[1:])):
        if lo <= indice <= hi or (i == len(labels)-1 and indice > lo):
            return labels[i]
    return labels[-1]

# Mantidas para compatibilidade com escadinha e proxima_bandeira (usam lista de labels)
BANDEIRAS_8 = ["A+","A","B+","B","C+","C","D+","D"]
BANDEIRAS_5 = ["A","B","C","D","E"]
BANDEIRAS_3 = ["A","B","C"]

# Cluster de exposição ao sol / formato (8 clusters, v14):
#  BK  -> KSK (loja com quiosque satélite aberto, via Database-Utilities) > RUA > EMPREENDIMENTO
#  PLK -> sempre EMPREENDIMENTO
#  SBUX-> QUIOSQUE (formato_area=KIOSK) > AEROPORTO > RUA > EMPREENDIMENTO
_BK_RUA      = {"FS","ILR"}
_SBUX_RUA    = {"STREET","HIGHWAY","UNIVERSITY"}
_SBUX_AERO   = {"AIRPORT"}

# Set de BKNs BK com KSK satélite aberto — preenchido em carregar_tudo() via Database-Utilities
_BKN_KSK: set = set()

def cluster_sol(marca, tipo_loja, tipo_area, bkn=None, formato_area=None):
    """Retorna o cluster nominal de uma loja (string maiúscula).
    bkn: BKN numérico (int ou str) — necessário para detectar KSK em BK.
    formato_area: campo do Cadastro_Lojas — necessário para detectar Quiosque SBUX."""
    marca=str(marca).upper(); tl=str(tipo_loja).upper()
    ta=str(tipo_area).upper(); fa=str(formato_area).upper() if formato_area else ""
    if marca=="BK":
        # KSK tem precedência: loja convencional que POSSUI quiosque satélite aberto
        if bkn is not None and str(bkn) in _BKN_KSK:
            return "KSK"
    ...
    if marca=="PLK":
        return "EMPREENDIMENTO"
    if marca=="SBUX":
        if "KIOSK" in fa:    return "QUIOSQUE"
...
def corrigir_indice_por_mediana(flr, rk):
    ...
    def _agg(g):
        ...
        prev=entra["custo_equivalente_previsto_score_rs"].dropna().sum()
    ...
    log(f"  indice corrigido em {time.time()-t:.1f}s; substituidos baixo(fantasma): {n_baixo}, "
        f"alto(energia/gas): {n_alto}; loja-mes sem indice: {n_semind}")
    return rk

def _carregar_ksk():
    """Preenche _BKN_KSK com BKNs BK que possuem quiosque satélite aberto.
    Fonte: Database-Utilities (PATH_SETOR), colunas Possui_KSK e Status_KSK.
    Falha silenciosa: se colunas ausentes, _BKN_KSK fica vazio e cluster KSK não é atribuído."""
    global _BKN_KSK
    if not os.path.exists(PATH_SETOR): return
    try:
        df=pd.read_excel(PATH_SETOR, sheet_name="Database", header=1)
        if "Possui_KSK" not in df.columns or "Status_KSK" not in df.columns:
            log("  aviso: colunas Possui_KSK/Status_KSK ausentes na Database — cluster KSK desativado")
            return
        ...
        def _ksk_aberto(r):
            p=str(r["Possui_KSK"]).strip().upper()
            s=str(r["Status_KSK"]).strip().upper()
            return ("SEM KSK" not in p and p not in ("NAN","")) and "ABERT" in s
        ksk=df[df.apply(_ksk_aberto,axis=1)]["BKN"].dropna()
        _BKN_KSK=set(ksk.tolist())
        log(f"  KSK carregados: {len(_BKN_KSK)} lojas BK com quiosque satélite aberto")
    except Exception as e:
        log(f"  aviso: erro ao carregar KSK ({e}) — cluster KSK desativado")

def _carregar_formato_area():
    """Retorna dict bkn -> formato_area do Cadastro_Lojas (para detectar SBUX Quiosque=KIOSK)."""
    try:
        cad=pd.read_excel(PATH_FATURA, sheet_name="Cadastro_Lojas")[["chave_loja","formato_area"]]
        cad["bkn"]=cad["chave_loja"].map(bkn_de_chave).astype(str).str.strip()
        return cad.set_index("bkn")["formato_area"].to_dict()
    except Exception as e:
        log(f"  aviso: formato_area indisponivel ({e}) — SBUX Quiosque pode nao ser detectado")
        return {}
...
def carregar_tudo():
    ...
    # 'bandeira' na fatura = MARCA (BK/PLK/SBUX); renomeia p/ nao colidir com nivel de eficiencia
    ...
    # v14: carrega KSK (cluster BK com quiosque satélite) e formato_area (SBUX Quiosque)
    _carregar_ksk()
    global _FORMATO_AREA_MAP
    _FORMATO_AREA_MAP=_carregar_formato_area()
...
_FORMATO_AREA_MAP: dict = {}  # bkn -> formato_area; preenchido em carregar_tudo()
...
def carregar_setores():
    """Carrega o mapa BKN -> Setor e BKN -> Regional_Operacao da base Database - Utilities.
    Setor e a subdivisao real dentro da Regional (a carteira). OBRIGATORIO para o painel
    do Gerente Regional (agrupa lojas por setor). Cabecalho real fica na 2a linha (header=1).
    Levanta erro claro se o arquivo nao existir."""
    ...
    df=pd.read_excel(PATH_SETOR, sheet_name="Database", header=1)
    df=df[[c for c in df.columns if str(c) in ("BKN","Setor","Regional_Operacao")]].copy()
    df["BKN"]=df["BKN"].astype(str).str.strip()
...
def carregar_alertas():
    ...
    Retorna dict bkn -> {'energia_dias':[...],'agua_dias':[...],'agua_continua':bool,'agua_ultimo_dia':int}."""
    out={}
    try:
        ...
        for bkn,g in en.groupby("BKN"):
            ...
            out.setdefault(b,{})["energia_dias"]=sorted(set(int(x) for x in g["Dia"].dropna()))
    except Exception as e:
...
def construir_pools_comparaveis(flr, ref_ts, medianas, temp_map=None):
    ...
    ref["cluster"]=ref.apply(lambda r: cluster_sol(r["marca"],r["tipo_loja"],r["tipo_area"]),axis=1)
...
def atribui_bandeiras(indice_series, faixas=None):
    """Mantida para compatibilidade interna (escadinha, proxima_bandeira).
    Para atribuição de nível usa-se nivel_absoluto_loja() ou nivel_absoluto_grupo()."""
...
def nivel_absoluto_loja(indice, marca, cluster):
    """Atribui nível absoluto (A+/A/B/C/D/E) a uma loja pelo seu cluster nominal.
    indice: escalar ou pd.Series. cluster: string retornada por cluster_sol()."""
    key=(str(marca).upper(), str(cluster).upper())
    if key not in REGUA_CLUSTER:
        key=("BK","EMPREENDIMENTO")  # fallback seguro
    edges, labels = REGUA_CLUSTER[key]
    return _nivel_absoluto(indice, edges, labels)

def nivel_absoluto_grupo(indice, marca):
    """Atribui nível absoluto ao grupo (Coordenador/Gerente) pelo índice médio da carteira."""
    m=str(marca).upper()
    if m not in REGUA_GRUPO: m="BK"
    edges, labels = REGUA_GRUPO[m]
    return _nivel_absoluto(indice, edges, labels)

def fronteiras_absolutas_cluster(marca, cluster):
    """Retorna dict {label -> corte_superior} para a régua absoluta do cluster.
    Usado pela escadinha e pelo Bloco Ação para saber a meta da próxima nota."""
    key=(str(marca).upper(), str(cluster).upper())
    if key not in REGUA_CLUSTER: key=("BK","EMPREENDIMENTO")
    edges, labels = REGUA_CLUSTER[key]
    # corte superior de cada faixa = edges[i+1]; última faixa não tem teto real
    return {labels[i]: edges[i+1] for i in range(len(labels)-1)}

def fronteiras_absolutas_grupo(marca):
    """Retorna dict {label -> corte_superior} para a régua absoluta de grupo."""
    m=str(marca).upper()
    if m not in REGUA_GRUPO: m="BK"
    edges, labels = REGUA_GRUPO[m]
    return {labels[i]: edges[i+1] for i in range(len(labels)-1)}
...
def bandeiras_por_cluster(ref_ts, flr, rk):
    """Atribui nível absoluto a cada loja dentro do seu cluster nominal (v14).
    Retorna: nivel_por_bkn, labels_por_bkn, fronteiras_por_cluster, cluster_por_bkn."""
    ...
    m["cluster"]=m.apply(lambda r: cluster_sol(
        r["marca"], r["tipo_loja"], r["tipo_area"],
        bkn=r["bkn"], formato_area=_FORMATO_AREA_MAP.get(r["bkn"],"")
    ), axis=1)
    nivel={}; labels_bkn={}; fronteiras={}; cluster_bkn={}
    for (marca,cluster),g in m.groupby(["marca","cluster"]):
        key=(marca,cluster)
        edges,labs=REGUA_CLUSTER.get(key, REGUA_CLUSTER[("BK","EMPREENDIMENTO")])
        s=g.set_index("bkn")["indice_eficiencia_mes"]
        nv=_nivel_absoluto(s, edges, labs)
        for bkn,fx in nv.items():
            nivel[bkn]=str(fx); labels_bkn[bkn]=labs; cluster_bkn[bkn]=(marca,cluster)
        front_sup=fronteiras_absolutas_cluster(marca, cluster)
        fronteiras[(marca,cluster)]={"faixas":labs, "fronteira": front_sup}
    return nivel, labels_bkn, fronteiras, cluster_bkn

def fronteiras_bandeira(indice_series, faixas=None):
    """Compatibilidade: retorna dict bandeira -> fronteira usando percentil.
    Substituída internamente pela régua absoluta; mantida para código externo que chame."""
...
def proxima_bandeira(atual, faixas=None):
    """Retorna o próximo nível (melhor) dentro da lista de labels do cluster.
    faixas: lista de labels do cluster, do melhor ao pior."""
    faixas=faixas or BANDEIRAS_8
...
def serie_consumo_12m(bkn, recurso, ref_ts, flr, hist, medianas=None, marca=None):
    """Serie de consumo real dos ultimos 12 meses. Omite meses sem leitura confiavel:
       - fora de FECHADO_COM_REAL
       - consumo fantasma (Regra A)
    NAO preenche por mediana (barra inventada). Historico so entra se existir de verdade."""
    meses=pd.date_range(end=ref_ts, periods=12, freq="MS"); val={}
    ...
    for _,r in sub.iterrows():
        c=r["consumo_real"]
        if consumo_fantasma(c, r.get("consumo_previsto"), med):   # Regra A: omite mes fantasma
            continue
        val[pd.Timestamp(r["competencia"]).strftime("%Y-%m")]=float(c)
    # historico (arquivo externo) so para meses ANTERIORES a base BI e se houver valor real
    if hist is not None and len(hist):
        ...
        for _,r in h.iterrows():
            ...
            if k not in val and pd.notna(r["consumo"]) and r["consumo"]>0:
                # aplica o mesmo crivo de fantasma contra a mediana da marca
                if med and r["consumo"] < 0.10*med: continue
                val.setdefault(k, float(r["consumo"]))
    # monta serie SO com meses que tem dado real (sem barras cinza)
    serie=[]
    for m in meses:
        k=m.strftime("%Y-%m")
        if k in val:
            serie.append((m.strftime("%b/%y"), val[k], False))
    return serie
...
def serie_indice_12m(bkn, ref_ts, rk, flr=None, medianas=None):
    """v12: usa o indice OFICIAL ja corrigido no rk (fantasma->mediana). NAO omite mais
    meses por fantasma: um mes so fica de fora quando o indice corrigido e NaN (todos os
    recursos fantasma sem mediana = loja sem indice naquele mes). Assim este grafico e o
    de 'Consumo em R$ x Meta' ficam consistentes e vao ate o mes de referencia."""
    ...
    return [(m.strftime("%b/%y"),float(val[m.strftime("%Y-%m")]))
            for m in meses if m.strftime("%Y-%m") in val]
...
def grafico_consumo(serie, recurso):
    ...
    labels=[s[0] for s in serie]; vals=[round(s[1]) for s in serie]
    return _chart({"type":"bar","data":{"labels":labels,"datasets":[{"data":vals,"backgroundColor":COR_REC[recurso]}]},
      "options":{"title":{"display":True,"text":f"{ROTULO[recurso]} ({UNIDADE[recurso]}/mes)"},
      "legend":{"display":False},"scales":{"yAxes":[{"ticks":{"beginAtZero":True}}]}}})
def grafico_indice(serie):
    if not serie: return ""
    labels=[s[0] for s in serie]; vals=[round(s[1]*100) for s in serie]; meta=[100]*len(labels)
    cores=[COR["verde"] if v<=100 else COR["vermelho"] for v in vals]
    # linha vermelha se o mes mais recente esta acima da meta, senao azul
    cor_linha=COR["vermelho"] if vals and vals[-1]>100 else COR["azul"]
    return _chart({"type":"line","data":{"labels":labels,"datasets":[
      {"label":"Indice de Consumo por Meta (%)","data":vals,"borderColor":cor_linha,"pointBackgroundColor":cores,"pointBorderColor":cores,"pointRadius":4,"fill":False},
      {"label":"Meta (100%)","data":meta,"borderColor":COR["cinza"],"borderDash":[6,4],"pointRadius":0,"fill":False}]},
      "options":{"title":{"display":True,"text":"Indice de Consumo por Meta (%) - abaixo de 100% e melhor"},
      "legend":{"display":True,"position":"bottom"}}},w=520,h=200)
...
def bloco_alerta_loja(bkn, alertas):
    ...
    if ag:
        if a.get("agua_continua"):
            sub='<span style="font-size:12px;color:'+COR["vermelho"]+';"><b>Provavel Vazamento de Agua: acione manutencao.</b></span>'
        else:
            sub='<span style="font-size:12px;color:'+COR["texto"]+';"><b>Provavel Vazamento de Agua, porem parou nos ultimos dias. Fique atento.</b></span>'
        linhas.append(
            f'<div><b style="color:{COR["azul"]};">Agua: Alertas de Alto Consumo na Madrugada:</b> {_formata_dias(ag)}<br>{sub}</div>')
...
def bloco_alerta_grupo(bkns, alertas, flr, ref_ts, max_lojas=None):
    ...
    for b in bkns:
        ...
        if a.get("agua_dias"):
            ag_rows.append((nome,len(a["agua_dias"]),a.get("agua_ultimo_dia",0),a.get("agua_continua",False)))
    ...
    if en_rows:
        li="".join(f'<li>{n}: {d} dias</li>' for n,d in en_rows)
    ...
    if ag_rows:
        li="".join(f'<li>{n}: {d} dias (ultimo dia {u:02d})</li>' for n,d,u,_ in ag_rows)
        ag_html=(f'<b style="color:{COR["azul"]};">Agua: Alertas de Alto Consumo na Madrugada &rarr; '
                 f'vazamentos muito provaveis:</b>'
                 f'<ul style="margin:6px 0 0 18px;padding:0;font-size:13px;">{li}</ul>')
    else:
...
def render_loja(d):
    ...
    <div style="font-weight:700;color:{COR['azul']};margin-bottom:8px;">Consumo em R$ x Meta - ultimos 12 meses</div>
    <div style="font-size:12px;color:{COR['cinza']};margin-bottom:8px;">Consumo dos 3 recursos convertido em R$ (tarifa de referencia) x meta. Alinha com o indice acima.</div>
    <div style="margin-bottom:16px;">{d['grafico_despesa_meta']}</div>
    <div style="font-weight:700;color:{COR['azul']};margin-bottom:8px;">Consumo dos ultimos 12 meses</div>
    {graf_consumo}
...
def tabelas_consumo_lado_a_lado(rank_consumo, grifadas=None, um_por_setor=False):
    ...
    lado a lado. v13.1: o usuario quer que a loja veja o consumo FISICO dela (acionavel), nao
    R$. Base = mesma do indice (consumo_real vs consumo_previsto); como custo = consumo x
    tarifa fixa, o % acima em consumo == % acima em custo, entao a correlacao com o indice se
    mantem. Ranqueado por % acima (comparavel entre lojas; rankings ja separados por
    marca+tipo). GRIFADAS (ofensoras) sempre incluidas nos 3 graficos (mesmo fora do top-10 e
    mesmo sem dado -> 'sem dado confiavel'), com fundo destacado e o mesmo numero (asterisco)
    das tabelas de indice. No Regional (um_por_setor) grifa 1 loja por setor."""
...
def tabela_promo_detra(promotoras, detratoras, grifadas=None, um_por_setor=False,
                       setor_por_bkn=None):
    """Tabelas lado a lado (v13.1): OFENSORAS (esq, indice > 100%, consome acima do
    esperado AGORA) e EFICIENTES (dir, indice <= 100%). O numero mostrado e o indice de
    consumo por meta da loja (100% = na meta). Lojas grifadas ganham fundo destacado.
    No Regional (um_por_setor=True) mostra 1 loja por setor."""
    grifadas=grifadas or set()
    ...
    def _linhas(lista, cor):
        if not lista: return f'<tr><td colspan="2" style="padding:6px;font-size:11px;color:{COR["cinza_claro"]};">Sem lojas neste grupo.</td></tr>'
        out=""
        for x in lista:
            ...
            indice=100+x["delta"]  # delta = indice - 100
            out+=(f'<tr><td style="padding:4px 8px;border:1px solid {COR["borda"]};font-size:11px;{bg}{bd}">{x["nome"]}{tag}</td>'
                  f'<td align="right" style="padding:4px 8px;border:1px solid {COR["borda"]};font-size:11px;color:{cor};font-weight:600;{bg}">'
                  f'{indice:.0f}%</td></tr>')
        return out
    ...
    return (f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr>'
            ...
            f'<div style="font-weight:700;font-size:12px;color:{COR["vermelho"]};margin-bottom:4px;">Lojas OFENSORAS <span style="font-weight:400;color:{COR["cinza"]};">(consomem acima do esperado)</span></div>'
            f'<table role="presentation" width="100%" style="border-collapse:collapse;">{detra}</table></td>'
            f'<td width="50%" valign="top" style="padding:4px;">'
            f'<div style="font-weight:700;font-size:12px;color:{COR["verde"]};margin-bottom:4px;">Lojas EFICIENTES <span style="font-weight:400;color:{COR["cinza"]};">(no esperado ou abaixo)</span></div>'
            f'<table role="presentation" width="100%" style="border-collapse:collapse;">{promo}</table></td>'
            f'</tr></table>')

def tabela_setores_mov(setores_mov, top=None):
    """Regional (v13.1): 'Setores OFENSORES' (esq, indice medio > 100%) e 'Setores
    EFICIENTES' (dir, <= 100%). Numero mostrado = indice medio do setor."""
    pior=setores_mov.get("pioraram",[]); melh=setores_mov.get("melhoraram",[])
    if top: pior=pior[:top]; melh=melh[:top]
    def _linhas(lista, cor):
    ...
    return (f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:6px;"><tr>'
            f'<td width="50%" valign="top" style="padding:4px;">'
            f'<div style="font-weight:700;font-size:12px;color:{COR["vermelho"]};margin-bottom:4px;">Setores OFENSORES <span style="font-weight:400;color:{COR["cinza"]};">(indice medio acima do esperado)</span></div>'
            f'<table role="presentation" width="100%" style="border-collapse:collapse;">{d}</table></td>'
            f'<td width="50%" valign="top" style="padding:4px;">'
            f'<div style="font-weight:700;font-size:12px;color:{COR["verde"]};margin-bottom:4px;">Setores EFICIENTES <span style="font-weight:400;color:{COR["cinza"]};">(no esperado ou abaixo)</span></div>'
...
def render_coordenador(d):
    ...
    <div style="margin-bottom:16px;">{d.get('grafico_indice','')}</div>
    <div style="font-weight:700;color:{COR['azul']};margin-bottom:8px;">Consumo em R$ x Meta - ultimos 12 meses</div>
    <div style="margin-bottom:16px;">{d.get('grafico_despesa_meta','')}</div>
...
def render_regional(d):
    ...
    {tabela_setores_mov(d.get('setores_mov',{'pioraram':[],'melhoraram':[]}))}
    {tabela_promo_detra(promotoras=d.get('promotoras',[]), detratoras=d.get('detratoras',[]), grifadas=grif, um_por_setor=True, setor_por_bkn=spb)}
    <div style="font-weight:700;color:{COR['azul']};margin:12px 0 6px;">Consumo acima do esperado por utility (% e unidade fisica)</div>
    {tabelas_consumo_lado_a_lado(d["rank_consumo"], grifadas=grif, um_por_setor=True)}
...
def dados_loja(bkn, ref_ts, prev_ts, flr, cmp, rk, hist, band_loja, fronteiras_cluster, band_loja_prev,
    ...
    cluster_alvo=meu_cl[1] if meu_cl else cluster_sol(marca, base.get("tipo_loja"), base.get("tipo_area"))
    ...
    return {"nome":base["nome"],"cidade":base["cidade"],"mes_label":ref_ts.strftime("%b/%Y"),
      ...
      "grafico_indice":grafico_indice(serie_indice_12m(bkn,ref_ts,rk,flr,medianas)),
      "grafico_despesa_meta":grafico_despesa_meta(serie_despesa_meta_12m(bkn,ref_ts,rk,medianas,marca)),
...
def montar_grupo(email, col_email, ref_ts, flr, rk, dp, indices_todos_grupos, front_grupos, p50_idx,
                 medianas=None, hist=None, setores_indice=None, cluster_loja=None):
    ...
    niv["desvio"]=(niv["indice_eficiencia_mes"]-1.0)*100  # >0 acima do esperado (ofensora)
    # ofensoras: indice > 100%, da que MAIS estoura para a que menos; eficientes: <= 100%,
    # da mais eficiente (menor indice) para a menos.
    detratoras=[{"nome":r["nome"],"delta":r["desvio"],"bkn":r["bkn"]}
                for _,r in niv[niv["desvio"]>0].sort_values("desvio",ascending=False).iterrows()]
    promotoras=[{"nome":r["nome"],"delta":r["desvio"],"bkn":r["bkn"]}
                for _,r in niv[niv["desvio"]<=0].sort_values("desvio").iterrows()]
    ...
    pos=int(serie.index.get_indexer([email])[0])+1 if email in serie.index else 0
    posicao_pares=f"{pos}o de {len(serie)}" if pos else f"- de {len(serie)}"
    ...
    # Acao acionavel (usa detratoras; retorna lojas p/ tabela)
    # BUGFIX v13: fronteiras agora e {marca: {bandeira: idx_max}} (bandeira calculada por
    # marca) - usa a fatia da marca do proprio grupo, nao mistura BK/PLK/SBUX.
    ...
    sim=simular_subida(bkns, ref_ts, rk, ind_grupo, faixa, fronteiras_marca, p50_idx, ref,
                       detratoras=detratoras, promotoras=promotoras, dp=dp,
                       cluster_por_bkn=cluster_loja)
...
def variacao_indice_por_setor(bkns, ref_ts, prev_ts, rk, setor_por_bkn):
    ...
    rows=[{"setor":s,"idx_now":float(v),"delta":float((v-1.0)*100)} for s,v in gn.items()]
    pioraram=sorted([r for r in rows if r["delta"]>0], key=lambda x:-x["delta"])
    melhoraram=sorted([r for r in rows if r["delta"]<=0], key=lambda x:x["delta"])
...
def simular_subida(bkns, ref_ts, rk, ind_atual, faixa_atual, fronteiras, p50_idx, ref,
                   detratoras=None, promotoras=None, dp=None, faixas_grupo=None,
                   cluster_por_bkn=None):
    """Acao acionavel: quais lojas precisam subir a PROPRIA nota para a carteira cruzar
    a proxima nota do grupo (v14: meta por loja = fronteira da proxima nota da loja,
    ancorada na régua absoluta do cluster dela — nao mais o p50 generico da rede).
    cluster_por_bkn: dict bkn -> (marca, cluster), usado para buscar a meta correta."""
    faixas=faixas_grupo or BANDEIRAS_8
    vazio={"texto":"", "lojas":[], "subiu_ok":False, "prox":None}
    if faixa_atual not in faixas: return vazio
    prox=proxima_bandeira(faixa_atual, faixas)
    if prox is None:
        return {"texto":"", "lojas":[], "subiu_ok":True, "prox":None}
    ...
    alvo=fronteiras.get(prox)
    if not alvo: return vazio
    nomes_map=ref[["bkn","nome"]].drop_duplicates("bkn").set_index("bkn")["nome"].to_dict()
    setor_map={}
    if dp is not None:
        sm=dp[["BKN","Regional"]].dropna()
        setor_map={str(b):s for b,s in zip(sm["BKN"].astype(str),sm["Regional"])}
    # meta individual de cada loja = fronteira superior da propria nota atual (prox nota da loja)
    # ex.: loja em C (1.00-1.16) -> meta e 1.00 (fronteira superior de B, a nota anterior a C)
    def meta_loja(bkn, idx_atual):
        if cluster_por_bkn and bkn in cluster_por_bkn:
            marca_l, cluster_l = cluster_por_bkn[bkn]
            front_l = fronteiras_absolutas_cluster(marca_l, cluster_l)
            labels_l = REGUA_CLUSTER.get((marca_l.upper(), cluster_l.upper()),
                       REGUA_CLUSTER[("BK","EMPREENDIMENTO")])[1]
            nivel_l = str(nivel_absoluto_loja(idx_atual, marca_l, cluster_l))
            prox_l = proxima_bandeira(nivel_l, labels_l)
            if prox_l and prox_l in front_l:
                return front_l[prox_l]
        # fallback: corte de B (1.00) como meta universal
        return 1.00
    # candidatos: lojas acima da propria meta (podem melhorar), mais ineficientes primeiro
    candidatos_bkns=[b for b in idx.index if idx[b] > meta_loja(b, idx[b])]
    candidatos=idx.loc[candidatos_bkns].sort_values(ascending=False)
    if detratoras:
        det_nomes=set(x["nome"] for x in detratoras)
        det_bkns=[b for b in candidatos.index if nomes_map.get(b) in det_nomes]
        outros=[b for b in candidatos.index if b not in det_bkns]
        ordem=det_bkns+outros
    else:
        ordem=list(candidatos.index)
    # simula: cada loja vai ate a propria meta (proxima nota dela)
    idx_sim=idx.copy(); usadas=[]
    for b in ordem:
        meta=meta_loja(b, idx[b])
        idx_sim[b]=min(idx_sim[b], meta); usadas.append(b)
        if idx_sim.mean()<=alvo: break
    subiu_ok=idx_sim.mean()<=alvo
    eco_total=0.0
    for b in usadas:
        i_at=idx.get(b); i_me=meta_loja(b, i_at)
    ...
    lojas=[{"nome":nomes_map.get(b,b),"bkn":b,"setor":setor_map.get(str(b),""),
            "delta_ind":None} for b in usadas]
    ...
    if subiu_ok and usadas:
        texto=(f'<b>Acao:</b> para voce subir de nivel de <b>{faixa_atual}</b> para '
               f'<b>{prox}</b>, as lojas <span style="background:{COR["grifo"]};padding:0 3px;'
               f'border-radius:3px;">grifadas</span> precisam subir de nivel '
               f'(reduzir o consumo por meta ate a fronteira da propria proxima nota).{eco_txt}')
    else:
        texto=(f'<b>Acao:</b> para subir de nivel de <b>{faixa_atual}</b> para <b>{prox}</b>, '
               f'foque nas lojas <span style="background:{COR["grifo"]};padding:0 3px;'
               f'border-radius:3px;">grifadas</span> (as que mais elevam o indice da carteira). '
               f'O salto exige melhora distribuida em varias delas.{eco_txt}')
...
def indices_por_grupo(col_email, ref_ts, prev_ts, flr, rk, dp, min_lojas):
    """Índice médio por grupo + nível absoluto por marca (v14).
    Nível do grupo = régua absoluta REGUA_GRUPO, ancorada em 1.00, por marca predominante.
    Fronteiras são os cortes fixos (não mais o max do percentil).
    Mantém separação por marca (BK/PLK/SBUX nunca se misturam na comparação)."""
    rk_ref=rk[rk["competencia"]==ref_ts][["bkn","indice_eficiencia_mes"]].dropna()
    ...
    marca_grupo=mm.groupby(col_email)["marca"].agg(
        lambda s: s.mode().iloc[0] if len(s.mode()) else None).to_dict()
    # nível absoluto por grupo: aplica REGUA_GRUPO pela marca predominante
    faixa=pd.Series(index=g.index, dtype=object)
    fronteiras={}
    for email in g.index:
        marca_g=marca_grupo.get(email,"BK")
        nv=nivel_absoluto_grupo(g.loc[email,"idx"], marca_g)
        faixa.loc[email]=str(nv)
        if marca_g not in fronteiras:
            fronteiras[marca_g]=fronteiras_absolutas_grupo(marca_g)
    # nível do mês anterior (mesmo critério absoluto)
    ...
    if len(gp):
        ...
        marca_grupo_prev=mpm.groupby(col_email)["marca"].agg(
            lambda s: s.mode().iloc[0] if len(s.mode()) else None).to_dict()
        for email in gp.index:
            mg=marca_grupo_prev.get(email,"BK")
            faixa_prev[email]=str(nivel_absoluto_grupo(gp.loc[email,"idx"], mg))
    nome_col=dp[["BKN",col_email,"Regional"]].rename(columns={"BKN":"bkn"})
    nome_col["bkn"]=nome_col["bkn"].astype(str)
    reg_grupo=nome_col.groupby(col_email)["Regional"].agg(
        lambda s: s.mode().iloc[0] if len(s.dropna().mode()) else "").to_dict()
    peers={e:{"idx":g.loc[e,"idx"],"marca":marca_grupo.get(e),"nome":reg_grupo.get(e,e)}
           for e in g.index}
    return g["idx"].to_dict(), {"faixa":faixa.to_dict(),"fronteiras":fronteiras,
                                "faixa_prev":faixa_prev,"peers":peers}
...
def main():
    ...
    if MODO_VALIDACAO:
        ...
        for email in coords:
            d=montar_grupo(email,"Email Coordenador",ref_ts,flr,rk,dp,idx_coord,front_coord,p50_idx,medianas,hist,setores_indice,cluster_loja=cluster_loja)
        ...
        for email in regs:
            d=montar_grupo(email,"Email Gerente Regional",ref_ts,flr,rk,dp,idx_reg,front_reg,p50_idx,medianas,hist,setores_indice,cluster_loja=cluster_loja)
