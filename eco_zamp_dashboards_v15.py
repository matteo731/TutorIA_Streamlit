...
ROTULO_RESUMO={"ENERGIA":"Energia","AGUA":"Agua","GAS":"Gas"}

def resumo_ofensoras_por_utility(rank_consumo, grifadas):
    """v15 (item novo, TEMA 6): para cada loja grifada (ofensora), lista qual(is) utility(ies)
    tornaram ela ofensora (pct>0 = consumo acima do esperado naquele recurso). Uma loja
    ofensora sempre tem >=1 utility acima da meta (definicao de ofensora e por consumo
    individual de cada utility), entao nao ha fallback para 'sem ofensor isolado'.
    Ordena pela mesma numeracao (num) usada na tabela Lojas OFENSORAS. O campo 'nome' ja
    vem como 'BKN - Nome da Loja' na fonte (flr), entao nao prefixa o bkn de novo."""
    if not grifadas: return []
    por_bkn={}
    for recurso in ["ENERGIA","AGUA","GAS"]:
        for row in rank_consumo.get(recurso,[]) or []:
            b=str(row.get("bkn",""))
            if b not in grifadas: continue
            if row.get("sem_dado"): continue
            if row.get("pct",0)>0:
                por_bkn.setdefault(b,{"bkn":b,"nome":row.get("nome",b),"num":row.get("num"),"utilities":[]})
                por_bkn[b]["utilities"].append(ROTULO_RESUMO[recurso])
    linhas=sorted(por_bkn.values(), key=lambda x:(x["num"] is None, x["num"] or 0))
    out=[]
    for x in linhas:
        uts=x["utilities"]
        if len(uts)==1: uts_txt=uts[0]
        elif len(uts)==2: uts_txt=f"{uts[0]} e {uts[1]}"
        else: uts_txt=", ".join(uts[:-1])+f" e {uts[-1]}"
        n_tag=f" ({x['num']})" if x["num"] else ""
        out.append(f"{x['nome']}{n_tag} = {uts_txt}")
    return out

def bloco_resumo_ofensoras(rank_consumo, grifadas):
    linhas=resumo_ofensoras_por_utility(rank_consumo, grifadas)
    if not linhas: return ""
    itens="".join(f'<div style="font-size:11px;color:{COR["texto"]};padding:2px 0;">{l}</div>' for l in linhas)
    return (f'<div style="background:{COR["cinza_bg"]};border-radius:8px;padding:8px 10px;margin-bottom:8px;">'
            f'<div style="font-weight:700;font-size:11px;color:{COR["azul"]};margin-bottom:4px;">Resumo</div>{itens}</div>')
...
def bloco_indice_nivel_setores(niveis_setor):
    """v15 (GR-1, TEMA 6): lista o indice atual e o Nivel de cada setor da regional,
    para acompanhar o grafico de indice das lojas. Nivel calculado ENTRE OS SETORES
    (esquema_faixas conforme o total de setores), entao usa as faixas do proprio
    tamanho da amostra de setores para colorir, nao BANDEIRAS_8 fixo."""
    if not niveis_setor: return ""
    faixas=esquema_faixas(len(niveis_setor))
    linhas="".join(
        f'<div style="font-size:11px;padding:2px 0;"><b>{x["setor"]}</b>: '
        f'{x["idx_now"]*100:.0f}% &middot; Nivel <b style="color:{cor_bandeira(x["nivel"],faixas)};">{x["nivel"]}</b></div>'
        for x in niveis_setor)
    return (f'<div style="background:{COR["cinza_bg"]};border-radius:8px;padding:8px 10px;margin-top:8px;">'
            f'<div style="font-weight:700;font-size:11px;color:{COR["azul"]};margin-bottom:4px;">Indice e Nivel por setor</div>{linhas}</div>')

def card_indice_medio_12m(indice_12m):
    """v15 (item 1, TEMA 6): card 'Indice Medio ultimos 12 meses', para ficar ao lado do
    grafico de indice de 12m. Puramente informativo/contextual: nao alimenta bandeira,
    pares, meta ou ranking (esses usam sempre o indice do mes de referencia)."""
    if indice_12m is None: return ""
    return f"""<table role="presentation" width="100%" height="100%" style="background:{COR['cinza_bg']};border-radius:10px;">
      <tr><td style="padding:14px 16px;" valign="middle">
      <div style="font-size:11px;color:{COR['cinza']};">INDICE MEDIO ULTIMOS 12 MESES</div>
      <div style="font-size:26px;font-weight:700;color:{COR['azul']};margin-top:2px;">{indice_12m*100:.0f}%</div>
      </td></tr></table>"""
...
def card_bandeira_grupo(d, tipo, is_regional=False):
    ...
    # cards do topo (v15: card INDICE MEDIO 12M removido daqui, reposicionado ao lado do
    # grafico de indice 12m em render_coordenador/render_regional. Fonte unica de indice
    # para bandeira/pares/meta/ranking/simulacao = ind_grupo (mes de referencia).)
    return f"""<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;"><tr>
      <td width="33%" style="padding:6px;"><table role="presentation" width="100%" style="background:{COR['verde_bg']};border-radius:10px;"><tr><td style="padding:12px 14px;">
      ...
      <td width="33%" style="padding:6px;"><table role="presentation" width="100%" style="background:{COR['cinza_bg']};border-radius:10px;"><tr><td style="padding:12px 14px;">
      ...
      <td width="33%" style="padding:6px;"><table role="presentation" width="100%" style="background:{COR['vermelho_bg']};border-radius:10px;"><tr><td style="padding:12px 14px;">
...
def render_coordenador(d):
    ...
    {bloco_resumo_ofensoras(d["rank_consumo"], grif)}
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:4px;"><tr>
      <td width="72%" valign="top">{d.get('grafico_indice','')}</td>
      <td width="28%" valign="middle" style="padding-left:10px;">{card_indice_medio_12m(d.get('indice_12m'))}</td>
    </tr></table>
    ...
    <div style="font-weight:700;color:{COR['azul']};margin-bottom:8px;">Consumo por Meta - ultimos 12 meses</div>
    {"".join(f'<div style="margin:10px 0;">{g}</div>' for g in d.get('graficos_consumo_grupo',[]))}
...
def render_regional(d):
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:4px;"><tr>
      <td width="72%" valign="top">{d.get('grafico_indice','')}</td>
      <td width="28%" valign="top" style="padding-left:10px;">
        {card_indice_medio_12m(d.get('indice_12m'))}
        {bloco_indice_nivel_setores(d.get('niveis_setor',[]))}
      </td>
    </tr></table>
    {("<div style='font-size:11px;color:"+chr(35)+"6b7a8d;margin-bottom:16px;'>Neste mes, "+d['legenda_movimentadores']+".</div>") if d.get('legenda_movimentadores') else ''}
    <div style="font-weight:700;color:{COR['azul']};margin-bottom:8px;">Consumo por Meta - ultimos 12 meses</div>
    {"".join(f'<div style="margin:10px 0;">{g}</div>' for g in d.get('graficos_consumo_grupo',[]))}
    {linha_fina()}
    <div style="font-weight:700;color:{COR['azul']};margin-bottom:4px;">Indice por Setor - ultimos 12 meses</div>
    <div style="font-size:11px;color:{COR['cinza']};margin-bottom:8px;">3 setores mais ofensores + 3 mais eficientes da regional.</div>
    <div style="margin-bottom:16px;">{d.get('grafico_indice_setor','')}</div>"""
    return html_doc(corpo)
...
def montar_grupo(email, col_email, ref_ts, flr, rk, dp, indices_todos_grupos, front_grupos, p50_idx,
    ...
    # v15 (item 2, TEMA 6): graficos "Consumo por Meta - ultimos 12 meses" do grupo,
    # mesmo formato do Dashboard de Loja (grouped bar Real vs Meta), 1 por recurso,
    # somando o consumo FISICO de todas as lojas da carteira/regional.
    graficos_consumo_grupo=[grafico_consumo(serie_consumo_12m_grupo(bkns,recurso,ref_ts,flr,medianas),recurso)
                            for recurso in ["ENERGIA","AGUA","GAS"]]
    ...
    if is_regional:
        ...
        # GR-2 (TEMA 6): corta em 3 AQUI, antes de somar com a simulacao, para que 'grifadas'
        # (usado nos graficos de consumo) seja EXATAMENTE o mesmo conjunto de 3 lojas que
        # aparece na tabela 'Lojas OFENSORAS' (top_ofens=3 em tabela_promo_detra). Sem este
        # corte, regionais com >3 setores ofensores estouravam o limite nos graficos.
        detra_1x1=detra_1x1[:3]
        bkns_detra_num=[str(x["bkn"]) for x in detra_1x1]
        # sim bkns que nao sao detratoras 1x1: adiciona 1 por setor tambem, so se sobrar espaco
        # dentro do limite de 3 (na pratica, quase nunca sobra, ja que as 3 detratoras mais
        # extremas costumam preencher os 3 slots).
        sim_extra=[b for b in bkns_sim if b not in bkns_detra_num]
        sim_extra_1x1=[]
        vistos={setor_por_bkn.get(b) for b in bkns_detra_num}
        for b in sim_extra:
            if len(bkns_detra_num)+len(sim_extra_1x1)>=3: break
            s=setor_por_bkn.get(b)
            if s in vistos: continue
            vistos.add(s); sim_extra_1x1.append(b)
        ordenadas=bkns_detra_num+sim_extra_1x1
        grifadas=set(ordenadas)  # no Regional, grifado = so as 3 (1/setor) selecionadas
    else:
    ...
    # v15 (GR-1/GR-3, TEMA 6): indice+Nivel atual por setor e grafico 12m por setor
    # (so calculado no Regional; is_regional definido logo acima nesta funcao)
    niveis_setor=nivel_setores_regional(bkns, ref_ts, rk, setor_por_bkn) if is_regional else []
    grafico_indice_setor=(grafico_indice_setores(serie_indice_12m_setores(bkns,ref_ts,rk,setor_por_bkn))
                          if is_regional else "")
    return {"nome":email,"n_lojas":len(bkns),"mes_label":ref_ts.strftime("%b/%Y"),
      ...
      "graficos_consumo_grupo":graficos_consumo_grupo,
      ...
      "niveis_setor":niveis_setor,"grafico_indice_setor":grafico_indice_setor,
      "prev_ts_g":prev_ts_g,"bkns":list(bkns)}

def serie_consumo_12m_grupo(bkns, recurso, ref_ts, flr, medianas=None):
    """v15 (item 2, TEMA 6): mesma logica de serie_consumo_12m, porem somando o consumo
    FISICO real e a meta (previsto corrigido) de todas as lojas do grupo, mes a mes.
    Reaproveita a mesma correcao de consumo fantasma e orcamento errado por loja.
    Usa a coluna 'marca' ja presente em flr (mesma fonte que dados_loja usa)."""
    meses=pd.date_range(end=ref_ts, periods=12, freq="MS")
    sub=flr[(flr["bkn"].isin(bkns))&(flr["recurso"]==recurso)&
            (flr["competencia"]<=ref_ts)&(flr["status_periodo"]=="FECHADO_COM_REAL")]
    val={}; prev={}
    for _,r in sub.iterrows():
        bkn=r["bkn"]; c=r["consumo_real"]
        med=(medianas or {}).get((r.get("marca"),recurso))
        if consumo_fantasma(c, r.get("consumo_previsto"), med):
            continue
        k=pd.Timestamp(r["competencia"]).strftime("%Y-%m")
        val[k]=val.get(k,0.0)+float(c)
        cp_corr=ORCAMENTO_ERRADO.get((str(bkn),recurso))
        cp=cp_corr if cp_corr is not None else r.get("consumo_previsto")
        if pd.notna(cp) and float(cp)>0:
            prev[k]=prev.get(k,0.0)+float(cp)
    serie=[]
    for m in meses:
        k=m.strftime("%Y-%m")
        if k in val:
...
_SETORES_INVALIDOS={"INATIVA","INATIVO","VAGO","DESCONHECIDO","NAN","NONE","","SEM_REGIONAL",
                    "SEM REGIONAL","SEM_SETOR","SEM SETOR","N/A","NA"}

def nivel_setores_regional(bkns, ref_ts, rk, setor_por_bkn):
    """v15 (GR-1, TEMA 6): indice medio atual e Nivel de cada SETOR da regional (nao das
    lojas). Nivel calculado ENTRE OS SETORES da propria regional (mesma regra de
    faixas por tamanho de amostra ja usada para lojas/grupos: 8/5/3 conforme o numero
    de setores). Retorna lista de {setor, idx_now, nivel} ordenada do pior pro melhor
    indice (mais ofensor primeiro)."""
    ...
    def _ok(s): return str(s).strip().upper() not in _SETORES_INVALIDOS and pd.notna(s)
    now=now[now["setor"].map(_ok)]
    if now.empty: return []
    gn=now.groupby("setor")["indice_eficiencia_mes"].mean()
    faixas=esquema_faixas(len(gn))
    niveis=atribui_bandeiras(gn, faixas)
    out=[{"setor":s,"idx_now":float(gn[s]),"nivel":str(niveis[s])} for s in gn.index]
    return sorted(out, key=lambda x:-x["idx_now"])

def serie_indice_12m_setores(bkns, ref_ts, rk, setor_por_bkn, top_ofens=3, top_efic=3):
    """v15 (GR-3, TEMA 6): serie de indice medio dos ultimos 12 meses, por SETOR da
    regional. Para legibilidade, seleciona apenas os top_ofens setores mais ofensores
    (maior indice no mes de referencia) e top_efic mais eficientes (menor indice),
    total <= top_ofens+top_efic. Retorna dict {setor: [(label, idx_medio_no_mes), ...]}
    na ordem: ofensores primeiro (mais ofensor->menos), depois eficientes (mais->menos)."""
    atual=nivel_setores_regional(bkns, ref_ts, rk, setor_por_bkn)
    if not atual: return {}
    ofensores=atual[:top_ofens]
    eficientes=list(reversed(atual[-top_efic:])) if len(atual)>top_ofens else []
    # evita duplicar setor se a regional tiver poucos setores (overlap ofensor/eficiente)
    vistos=set(); selecionados=[]
    for x in ofensores+eficientes:
        if x["setor"] in vistos: continue
        vistos.add(x["setor"]); selecionados.append(x["setor"])
    now=rk[(rk["competencia"]<=ref_ts)&(rk["bkn"].isin(bkns))][["bkn","competencia","indice_eficiencia_mes"]].dropna()
    now["setor"]=now["bkn"].astype(str).map(setor_por_bkn)
    meses=pd.date_range(end=ref_ts, periods=12, freq="MS")
    out={}
    for s in selecionados:
        ...
        serie=[]
        for m in meses:
            mm=sub[sub["competencia"]==m]["indice_eficiencia_mes"]
            if len(mm): serie.append((m.strftime("%b/%y"), float(mm.mean())))
        if serie: out[s]=serie
    return out

def grafico_indice_setores(series_por_setor):
    """v15 (GR-3): grafico de linha com ate 6 series (uma por setor selecionado), sem
    grade, cada setor com cor propria. Sem datalabel individual por ponto (6 series
    lado a lado ficaria ilegivel); a legenda ja identifica cada setor."""
    if not series_por_setor: return ""
    cores=["#2f6fed","#e0554f","#2fb380","#e0a02f","#8a5fd6","#3fb8c2"]
    labels=None; datasets=[]
    for i,(setor,serie) in enumerate(series_por_setor.items()):
        if labels is None: labels=[s[0] for s in serie]
        datasets.append({"label":setor,"data":[round(s[1]*100) for s in serie],
                         "borderColor":cores[i%len(cores)],"pointRadius":2,"fill":False,
                         "datalabels":{"display":False}})
    meta=[100]*len(labels)
    datasets.append({"label":"Meta (100%)","data":meta,"borderColor":COR["cinza"],
                     "borderDash":[6,4],"pointRadius":0,"fill":False,"datalabels":{"display":False}})
    return _chart({"type":"line","data":{"labels":labels,"datasets":datasets},
      "options":{"title":{"display":True,"text":"Indice por Setor (%) - 3 mais ofensores + 3 mais eficientes - ultimos 12 meses"},
      "legend":{"display":True,"position":"bottom"},
      "scales":{"xAxes":[{"gridLines":{"display":False}}],
                "yAxes":[{"gridLines":{"display":False}}]}}},w=560,h=260)
