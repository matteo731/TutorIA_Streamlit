...
def serie_consumo_12m(bkn, recurso, ref_ts, flr, hist, medianas=None, marca=None):
    """v14: Serie de 12 meses para o grafico de consumo da LOJA, expandido para incluir
    meses futuros (PROJECAO) alem dos meses passados (FECHADO_COM_REAL).

    Retorna lista de tuplas:
        (label_mes, consumo, eh_projecao, meta)
    onde:
        consumo     = consumo_real (meses fechados) ou consumo_projecao_fechamento (PROJECAO)
        eh_projecao = False para meses fechados, True para meses com status PROJECAO
        meta        = consumo_previsto (meses fechados, jan-ref_ts) ou
                      consumo_orcado  (meses futuros, apos ref_ts)
                      -- None quando nenhum dos dois disponivel

    Regras de inclusao:
    - Meses FECHADO_COM_REAL: inclui se consumo_real nao eh fantasma (Regra A). Meta=previsto.
    - Meses PROJECAO (status_periodo=='PROJECAO'): inclui sempre que
      consumo_projecao_fechamento estiver disponivel. Meta=consumo_orcado.
    - Historico externo (hist): mesma regra de antes, apenas para meses sem dado na base BI.
    - Janela: 12 meses terminando em ref_ts MAIS meses de status PROJECAO ate dez do mesmo ano.
    """
    ref_ts=pd.Timestamp(ref_ts)
    ano_ref=ref_ts.year
    fim_ano=pd.Timestamp(f"{ano_ref}-12-01")

    # Janela: 12 meses passados/presentes + meses futuros ate dezembro do mesmo ano
    meses_passados=pd.date_range(end=ref_ts, periods=12, freq="MS")
    meses_futuros=pd.date_range(start=ref_ts+pd.offsets.MonthBegin(1), end=fim_ano, freq="MS")
    todos_meses=meses_passados.append(meses_futuros)

    sub=flr[(flr["bkn"]==bkn)&(flr["recurso"]==recurso)]
    med=(medianas or {}).get((marca,recurso))

    val={}  # chave "YYYY-MM" -> (consumo, eh_projecao, meta)

    # -- meses fechados (FECHADO_COM_REAL / FECHADO_SEM_REAL) --
    fechados=sub[sub["status_periodo"].isin(["FECHADO_COM_REAL","FECHADO_SEM_REAL"])]
    for _,r in fechados.iterrows():
        c=r["consumo_real"]
        if pd.isna(c): continue
        if consumo_fantasma(c, r.get("consumo_previsto"), med):  # Regra A
            continue
        meta_val=r.get("consumo_previsto")
        meta_val=float(meta_val) if pd.notna(meta_val) else None
        val[pd.Timestamp(r["competencia"]).strftime("%Y-%m")]=(float(c), False, meta_val)

    # -- historico externo (meses anteriores a base BI) --
    if hist is not None and len(hist):
        ...
        for _,r in h.iterrows():
            ...
            if k not in val and pd.notna(r["consumo"]) and r["consumo"]>0:
                ...
                val.setdefault(k,(float(r["consumo"]), False, None))

    # -- meses futuros (PROJECAO) --
    projecoes=sub[sub["status_periodo"]=="PROJECAO"]
    for _,r in projecoes.iterrows():
        proj=r.get("consumo_projecao_fechamento")
        if pd.isna(proj): continue
        meta_val=r.get("consumo_orcado")
        meta_val=float(meta_val) if pd.notna(meta_val) else None
        val[pd.Timestamp(r["competencia"]).strftime("%Y-%m")]=(float(proj), True, meta_val)

    # -- monta serie na ordem cronologica --
    serie=[]
    for m in todos_meses:
        k=m.strftime("%Y-%m")
        if k in val:
            consumo, eh_proj, meta=val[k]
            serie.append((m.strftime("%b/%y"), consumo, eh_proj, meta))
    return serie
...
def calc_serie_p25_cluster(bkn, ref_ts, flr, rk, cluster_por_bkn=None, medianas=None, marca=None):
    """v14: Calcula o indice equivalente ao P25 do cluster para cada mes dos ultimos 12.
    Logica: para cada mes, soma (consumo_benchmark_p25 x tarifa_orcada) dos recursos
    ativos da loja (consumo_previsto > 0) e divide pela soma (consumo_previsto x tarifa_orcada).
    Isso coloca o P25 fisico na mesma escala adimensional do indice de eficiencia.
    Retorna lista de (label, indice_p25) apenas para meses com dado valido.
    """
    ref_ts=pd.Timestamp(ref_ts)
    meses=pd.date_range(end=ref_ts, periods=12, freq="MS")
    sub=flr[(flr["bkn"]==bkn)&(flr["competencia"]<=ref_ts)]
    serie=[]
    for m in meses:
        rows=sub[sub["competencia"]==m]
        if rows.empty: continue
        num=0.0; den=0.0
        for _,r in rows.iterrows():
            p25=r.get("consumo_benchmark_p25"); prev=r.get("consumo_previsto")
            tarifa=r.get("tarifa_orcada")
            if pd.isna(p25) or pd.isna(prev) or pd.isna(tarifa): continue
            if prev<=0: continue  # recurso inativo, nao entra na soma
            num+=float(p25)*float(tarifa)
            den+=float(prev)*float(tarifa)
        if den>0:
            serie.append((m.strftime("%b/%y"), num/den))
    return serie


def calc_serie_melhor_cluster(bkn, ref_ts, rk, cluster_por_bkn=None):
    """v14: Envelope dinamico -- para cada mes dos ultimos 12, retorna o indice da loja
    com menor indice_eficiencia_mes dentro do mesmo cluster da loja-alvo.
    'Mesmo cluster' = mesma tupla (marca, cluster) de cluster_por_bkn.
    A melhor loja pode ser diferente a cada mes (envelope, nao uma loja fixa).
    Retorna lista de (label, indice_melhor) apenas para meses com dado valido.
    Exclui a propria loja do calculo para nao distorcer quando ela e a melhor.
    """
    ref_ts=pd.Timestamp(ref_ts)
    meses=pd.date_range(end=ref_ts, periods=12, freq="MS")
    meu_cluster=(cluster_por_bkn or {}).get(bkn)
    if meu_cluster is None: return []

    # bkns do mesmo cluster (exceto a propria loja)
    pares=[b for b,cl in (cluster_por_bkn or {}).items() if cl==meu_cluster and b!=bkn]
    if not pares: return []

    sub=rk[(rk["bkn"].isin(pares))&(rk["competencia"]<=ref_ts)]
    serie=[]
    for m in meses:
        # filtra indice >= 0.10 E cobertura_score >= 0.5 para excluir lojas com dado
        # incompleto ou fantasma -- mesmo crivo usado em bandeiras_por_cluster (linha ~831)
        rows=sub[(sub["competencia"]==m)&(sub["indice_eficiencia_mes"].notna())
                 &(sub["indice_eficiencia_mes"]>=0.10)
                 &(sub["cobertura_score"]>=0.5)]
        if rows.empty: continue
        melhor=rows["indice_eficiencia_mes"].min()
        serie.append((m.strftime("%b/%y"), float(melhor)))
    return serie
...
def grafico_consumo(serie, recurso):
    """v14: Grafico de consumo da LOJA expandido para jan-dez.
    serie: lista de (label, consumo, eh_projecao, meta) gerada por serie_consumo_12m.
    - Meses passados (eh_projecao=False): barra na cor do recurso (real).
    - Meses futuros (eh_projecao=True): barra mais clara/transparente (projecao ML).
    - Meta: linha tracejada cinza usando consumo_previsto (passado) ou consumo_orcado (futuro).
    Compatibilidade retroativa: aceita tuplas de 2 ou 3 elementos (legado coordenador/gerente).
    """
    if not serie:
    ...
    # compatibilidade retroativa com serie de 2 ou 3 elementos
    if len(serie[0]) <= 3:
    ...
    # nova logica: 4 elementos (label, consumo, eh_projecao, meta)
    labels=[s[0] for s in serie]
    cor_base=COR_REC[recurso]

    # barras reais (meses fechados)
    vals_real=[round(s[1]) if not s[2] else None for s in serie]
    # barras projecao ML (meses futuros) -- cor mais clara via alpha no hex
    vals_proj=[round(s[1]) if s[2] else None for s in serie]
    # linha de meta (previsto ou orcado conforme o mes)
    vals_meta=[round(s[3]) if s[3] is not None else None for s in serie]

    # cor clara para projecao: aplica 60% de opacidade simulado com hex mais claro
    # usa a cor base do recurso misturada com branco -- aproximacao via literal
    cor_proj_map={"ENERGIA":"#f3c88e","AGUA":"#8ec3f3","GAS":"#f3a08e"}
    cor_proj=cor_proj_map.get(recurso, "#c0c0c0")

    datasets=[
        {"label":"Consumo real","data":vals_real,"backgroundColor":cor_base,"order":2},
        {"label":"Projecao ML","data":vals_proj,"backgroundColor":cor_proj,"order":2},
        {"label":"Meta","data":vals_meta,"type":"line","borderColor":COR["cinza"],
         "borderDash":[5,4],"pointRadius":2,"fill":False,"borderWidth":2,"order":1},
    ]
    tem_proj=any(s[2] for s in serie)
    return _chart({"type":"bar","data":{"labels":labels,"datasets":datasets},
      "options":{"title":{"display":True,"text":f"{ROTULO[recurso]} ({UNIDADE[recurso]}/mes) - Real vs Meta vs Projecao"},
      "legend":{"display":tem_proj,"position":"bottom"},
      "scales":{"yAxes":[{"ticks":{"beginAtZero":True}}]}}},w=520,h=200)
def grafico_indice(serie, serie_p25=None, serie_melhor=None):
    """v14: Grafico de indice de eficiencia da LOJA com linhas de referencia opcionais.

    Parametros:
        serie        : lista de (label, indice_float) -- sempre presente.
        serie_p25    : lista de (label, indice_p25_float) -- indice equivalente ao P25 do cluster
                       por mes. None = nao plota. Veja calc_serie_p25_cluster().
        serie_melhor : lista de (label, indice_melhor_float) -- indice da melhor loja do cluster
                       naquele mes (envelope dinamico). None = nao plota.
                       Veja calc_serie_melhor_cluster().

    TODO (Item 1.1 -- nivel por mes): adicionar rotulos de nivel (A, A+, B...) sobre cada
    ponto da linha principal, recalculando as fronteiras de bandeira MES A MES com a carteira
    daquele mes (nao so do mes de referencia). Isso exige que bandeiras_por_cluster() seja
    chamada para cada competencia historica, nao apenas para ref_ts. A implementacao atual
    usa apenas o nivel do mes de referencia; o nivel historico requer refactor da funcao
    render_loja_dados() para passar fronteiras_por_competencia ao inves de fronteiras_cluster.
    Regra de exibicao decidida: rotular apenas quando o nivel MUDA em relacao ao mes anterior
    (transicoes), nao todos os 12 pontos, para evitar poluicao visual em 520px.
    """
    if not serie: return ""
    ...
    datasets=[
        {"label":"Indice de Consumo por Meta (%)","data":vals,
         "borderColor":cor_linha,"pointBackgroundColor":cores,"pointBorderColor":cores,
         "pointRadius":4,"fill":False,"order":1},
        {"label":"Meta (100%)","data":meta,"borderColor":COR["cinza"],
         "borderDash":[6,4],"pointRadius":0,"fill":False,"order":3},
    ]

    # linha P25 do cluster (indice equivalente ao patamar eficiente dinamico)
    if serie_p25:
        p25_map={s[0]:round(s[1]*100,1) for s in serie_p25}
        p25_vals=[p25_map.get(lb) for lb in labels]
        datasets.append({
            "label":"P25 do cluster","data":p25_vals,
            "borderColor":"#1a7a45","borderDash":[3,3],
            "pointRadius":0,"fill":False,"borderWidth":1.5,"order":2
        })

    # linha melhor loja do cluster (envelope dinamico: melhor de cada mes)
    if serie_melhor:
        melhor_map={s[0]:round(s[1]*100,1) for s in serie_melhor}
        melhor_vals=[melhor_map.get(lb) for lb in labels]
        datasets.append({
            "label":"Melhor loja do cluster","data":melhor_vals,
            "borderColor":"#7b3fa0","borderDash":[4,2],
            "pointRadius":0,"fill":False,"borderWidth":1.5,"order":2
        })

    tem_ref=bool(serie_p25 or serie_melhor)
    return _chart({"type":"line","data":{"labels":labels,"datasets":datasets},
      ...
      "legend":{"display":True,"position":"bottom"}}},w=520,h=200 if not tem_ref else 220)
...
def dados_loja(bkn, ref_ts, prev_ts, flr, cmp, rk, hist, band_loja, fronteiras_cluster, band_loja_prev,
    ...
    return {"nome":base["nome"],"cidade":base["cidade"],"mes_label":ref_ts.strftime("%b/%Y"),
      ...
      "grafico_indice":grafico_indice(
          serie_indice_12m(bkn,ref_ts,rk,flr,medianas),
          serie_p25=calc_serie_p25_cluster(bkn,ref_ts,flr,rk,cluster_loja,medianas,marca),
          serie_melhor=calc_serie_melhor_cluster(bkn,ref_ts,rk,cluster_loja),
      ),
