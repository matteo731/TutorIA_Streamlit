...
def comparavel_por_recurso(alvo_bkn, marca, recurso, consumo_alvo, trafego_alvo, despesa_alvo,
    ...
    Filtros preferenciais: area +-10% (CRITERIO PRINCIPAL), trafego +-20% (desempate/relaxamento),
    (energia rua) temperatura +-2C. Relaxa progressivamente para garantir pelo menos 1 par (a
    loja SEMPRE tem par se existir alguem no cluster consumindo menos), como pedido: o
    obrigatorio e ter par.
    v13.4: area passa a ser o criterio principal (consumo de utility varia mais com area do
    que com trafego); trafego so decide em caso de desempate ou quando nao ha par por area.
    Validado com dados reais: cobertura de pares identica (98,95% nos dois modos, nenhuma
    loja perde par); dentro do bloco que hoje cai no nivel intermediario (~27% dos casos, sem
    par com as 2 variaveis dentro da tolerancia), a fatia com area dentro da tolerancia sobe
    de 1,7% para 14,6% e a fatia so-trafego cai de 25,4% para 12,5%. Os pares 'otimos'
    (area+trafego OK, 69,9%) nao mudam."""
    ...
    def aplica(cand, usar_traf, usar_area, usar_temp_f):
        ...
        return c
    # v13.4: area sai por ultimo no relaxamento (criterio principal); trafego e o primeiro a
    # ser solto quando nao ha par com as 2 variaveis dentro da tolerancia.
    tentativas=[
        (True,  True,  usa_temp),   # tudo
        (False, True,  usa_temp),   # sem trafego (mantem area+temp)
        (True,  True,  False),      # sem temp (mantem area+trafego)
    ...
    # escolhe o de area mais proxima (ou trafego, se area ausente) - desempate v13.4
    if area_alvo and area_alvo>0:
        cand=cand.assign(dist=(cand["area_vendas_m2"]-area_alvo).abs())
    elif trafego_alvo and trafego_alvo>0:
    ...
    if prox and indice_mes is not None and not ind_row.empty:
        ...
        if alvo_idx and indice_mes>alvo_idx:
            ...
            if pd.notna(real) and pd.notna(prev_c) and prev_c>0:
                ...
                # arredondamento visual: garante pelo menos 1pp de diferenca entre os 2
                # valores exibidos (evita "de 100% para 100%" quando os indices reais sao
                # proximos). Nao altera o calculo de reducao_rs, so a exibicao.
                p_atual=round(indice_mes*100); p_alvo=round(alvo_idx*100)
                if p_alvo>=p_atual: p_alvo=p_atual-1
                meta_txt=(f'Para a bandeira <b>{prox}</b>: reduzir o indice de {p_atual:.0f}% para {p_alvo:.0f}%, '
