...
def render_loja(d):
    ...
    {bloco_impacto_ambiental(d.get('recursos',{}))}"""
...
def dados_loja(bkn, ref_ts, prev_ts, flr, cmp, rk, hist, band_loja, fronteiras_cluster, band_loja_prev,
               ...
               texto_ranking=None, ego_lojas=None):
    ...
    for recurso in ["ENERGIA","AGUA","GAS"]:
        ...
        # economia fisica (v14): previsto corrigido - real, so positivo = economizou.
        # usa o mesmo previsto corrigido (ORCAMENTO_ERRADO) que o resto do pipeline.
        economia_fisica=0.0
        if not fantasma and pd.notna(r.get("consumo_previsto")):
            cp_corr=ORCAMENTO_ERRADO.get((str(bkn),recurso), r.get("consumo_previsto"))
            if pd.notna(cp_corr) and consumo_val is not None:
                economia_fisica=float(cp_corr)-float(consumo_val)
        recursos[recurso]={"consumo":consumo_val,"delta":delta,"despesa":desp,
                           ...
                           "economia_fisica":economia_fisica}
    ...
    # ---- Ego / autoimagem (v14, broadcast) ----
    frases_ego=[]
    if ego_lojas:
        frases_ego=frases_ego_loja(bkn, marca, meu_cluster,
                                   ego_lojas.get("melhor_cluster",{}), ego_lojas.get("melhor_marca",{}),
                                   ego_lojas.get("mais_melhorou",{}), ego_lojas.get("nomes",{}))
    return {"nome":base["nome"],"cidade":base["cidade"],"mes_label":ref_ts.strftime("%b/%Y"),
      ...
      "bloco_ego":bloco_ego_loja(frases_ego),
...
# ============================ IMPACTO AMBIENTAL (v14, so LOJA) ============================
# Regras: so entra recurso com economia real (consumo_real < consumo_previsto corrigido) e
# dado confiavel (nao fragil, nao fantasma). Bloco inteiro some se nenhum recurso economizou.
FATOR_ENERGIA_KWH_CASA_MES = 160.0     # EPE: consumo medio residencial mensal
FATOR_AGUA_L_POR_COPO = 0.5            # copo grande (500ml)
FATOR_GAS_KG_POR_BOTIJAO = 13.0        # botijao P13
FATOR_GAS_DIAS_POR_BOTIJAO = 40.0      # media de uso domestico (35-45 dias)

def _formata_anos_meses(meses_int):
    if meses_int<1: return None
    anos, resto = divmod(meses_int, 12)
    if anos==0:
        return f"{resto} mes" if resto==1 else f"{resto} meses"
    if resto==0:
        return f"{anos} ano" if anos==1 else f"{anos} anos"
    txt_ano = "1 ano" if anos==1 else f"{anos} anos"
    txt_mes = "1 mes" if resto==1 else f"{resto} meses"
    return f"{txt_ano} e {txt_mes}"

def impacto_ambiental_loja(recursos):
    """Retorna lista de frases de impacto ambiental (uma por recurso que economizou de
    fato, com dado confiavel). recursos: dict recurso->info, ja vindo de dados_loja,
    precisa ter 'economia_fisica' (consumo_previsto - consumo_real, >0 se economizou)."""
    frases=[]
    # ENERGIA -> casas abastecidas por 1 mes
    e=recursos.get("ENERGIA")
    if e and not e.get("fragil") and not e.get("fantasma") and e.get("economia_fisica",0)>0:
        casas=e["economia_fisica"]/FATOR_ENERGIA_KWH_CASA_MES
        if casas>=0.1:
            frases.append(f"Sua loja economizou energia suficiente para abastecer <b>{casas:.1f} casas</b> por um mes inteiro!")
    # AGUA -> copos grandes de refrigerante (500ml)
    a=recursos.get("AGUA")
    if a and not a.get("fragil") and not a.get("fantasma") and a.get("economia_fisica",0)>0:
        litros=a["economia_fisica"]*1000.0  # consumo em m3 -> litros
        copos=litros/FATOR_AGUA_L_POR_COPO
        if copos>=1:
            frases.append(f"A agua economizada equivale a <b>{copos:,.0f} copos</b> grandes de refrigerante ate a borda!".replace(",","."))
    # GAS -> tempo de autonomia de um botijao P13 para uma familia
    g=recursos.get("GAS")
    if g and not g.get("fragil") and not g.get("fantasma") and g.get("economia_fisica",0)>0:
        botijoes=g["economia_fisica"]/FATOR_GAS_KG_POR_BOTIJAO
        meses_exato=botijoes*(FATOR_GAS_DIAS_POR_BOTIJAO/30.0)
        meses_int=round(meses_exato)
        txt=_formata_anos_meses(meses_int)
        if txt:
            frases.append(f"E gas suficiente para abastecer a cozinha de uma familia inteira por cerca de <b>{txt}</b>!")
    return frases

def bloco_impacto_ambiental(recursos):
    frases=impacto_ambiental_loja(recursos)
    if not frases: return ""
    linhas="".join(f'<div style="margin-top:4px;">&#127793; {f}</div>' for f in frases)
    return f"""<div style="margin-top:18px;background:{COR['verde_bg']};border-radius:10px;padding:14px 18px;">
      <div style="font-weight:700;color:{COR['verde']};font-size:13px;margin-bottom:2px;">IMPACTO AMBIENTAL DO MES</div>
      <div style="font-size:13px;color:{COR['texto']};">{linhas}</div></div>"""

# ============================ EGO / AUTOIMAGEM (v14) ============================
def nome_de_email(email):
    """Extrai o nome de exibicao do e-mail: tudo antes do '@'."""
    if not email or "@" not in str(email): return str(email or "-")
    return str(email).split("@")[0]

def frase_ego(vencedor_txt_venceu, vencedor_txt_resto, quem_sou_eu, vencedor_nome):
    """Frase dupla: quem venceu ve parabens, os demais veem o nome do vencedor."""
    if quem_sou_eu==vencedor_nome:
        return vencedor_txt_venceu
    return vencedor_txt_resto

def ranking_ego_lojas(ref_ts, flr, rk, cluster_por_bkn, band_loja):
    """Pre-calculo (1x por rodada) dos vencedores de ego a nivel Loja:
    - melhor loja de cada (marca,cluster) [ja e o 'Nivel A/A+' do proprio cluster]
    - melhor loja de cada marca (Brasil, dentro da marca, nunca cruza marca)
    - loja que mais melhorou o indice no mes (delta vs mes anterior), por marca
    Retorna dict com bkn->nome para uso nas frases."""
    rk_ref=rk[rk["competencia"]==ref_ts][["bkn","indice_eficiencia_mes"]].dropna()
    info=flr[flr["competencia"]==ref_ts][["bkn","marca","nome"]].drop_duplicates("bkn")
    m=rk_ref.merge(info,on="bkn",how="left").dropna(subset=["marca"])
    m["cluster"]=m["bkn"].map(lambda b: cluster_por_bkn.get(b))
    melhor_cluster={}  # (marca,cluster) -> bkn
    for (marca,cluster),g in m.groupby(["marca","cluster"]):
        if g.empty: continue
        best=g.loc[g["indice_eficiencia_mes"].idxmin()]
        melhor_cluster[(marca,cluster)]=str(best["bkn"])
    melhor_marca={}  # marca -> bkn (melhor de toda a marca no Brasil)
    for marca,g in m.groupby("marca"):
        if g.empty: continue
        best=g.loc[g["indice_eficiencia_mes"].idxmin()]
        melhor_marca[marca]=str(best["bkn"])
    return {"melhor_cluster":melhor_cluster,"melhor_marca":melhor_marca,
            "nomes":info.set_index("bkn")["nome"].astype(str).to_dict(),
            "marca_por_bkn":info.set_index("bkn")["marca"].astype(str).to_dict()}

def loja_que_mais_melhorou(ref_ts, prev_ts, flr, rk):
    """Loja com maior queda de indice (melhora) no mes, por marca. Retorna marca->bkn."""
    cur=rk[rk["competencia"]==ref_ts][["bkn","indice_eficiencia_mes"]].dropna().rename(
        columns={"indice_eficiencia_mes":"cur"})
    prev=rk[rk["competencia"]==prev_ts][["bkn","indice_eficiencia_mes"]].dropna().rename(
        columns={"indice_eficiencia_mes":"prev"})
    info=flr[flr["competencia"]==ref_ts][["bkn","marca"]].drop_duplicates("bkn")
    m=cur.merge(prev,on="bkn").merge(info,on="bkn",how="left").dropna(subset=["marca"])
    m["delta"]=m["cur"]-m["prev"]  # negativo = melhorou
    out={}
    for marca,g in m.groupby("marca"):
        if g.empty: continue
        best=g.loc[g["delta"].idxmin()]
        if best["delta"]<0: out[marca]=str(best["bkn"])
    return out

def ranking_ego_coordenadores(idx_coord, front_coord, peers_marca):
    """Pre-calculo de ego a nivel Coordenador:
    - recorde proprio (12m): calculado por email na hora (precisa historico, feito em montar_grupo)
    - salto de Nivel (faixa atual vs anterior, melhorou) -> ja disponivel via front_coord['faixa_prev']
    - podio (top 1/2/3) do cluster/marca -> ranking por marca do idx atual
    - maior salto de indice no mes -> delta entre idx atual e idx do mes anterior (medio)
    """
    faixa=front_coord.get("faixa",{}); faixa_prev=front_coord.get("faixa_prev",{})
    podio_por_marca={}  # marca -> [email1(1o),email2(2o),email3(3o)]
    por_marca={}
    for email,marca in peers_marca.items():
        por_marca.setdefault(marca,[]).append(email)
    for marca,emails in por_marca.items():
        ordenado=sorted(emails, key=lambda e: idx_coord.get(e,1e9))
        podio_por_marca[marca]=ordenado[:3]
    salto_nivel={}  # email -> True se subiu de faixa vs mes anterior
    ordem_faixas=BANDEIRAS_8  # ordem do melhor pro pior; funciona para qualquer subconjunto usado
    for email,fx_atual in faixa.items():
        fx_ant=faixa_prev.get(email)
        if fx_ant is None or fx_atual is None or fx_ant==fx_atual: continue
        try:
            if ordem_faixas.index(fx_atual)<ordem_faixas.index(fx_ant):
                salto_nivel[email]=(fx_ant,fx_atual)
        except ValueError:
            pass
    return {"podio_por_marca":podio_por_marca,"salto_nivel":salto_nivel}

def ranking_ego_regionais(idx_reg, front_reg, peers_marca, dp, ref_ts, prev_ts, rk, band_loja, band_loja_prev):
    """Pre-calculo de ego a nivel Gerente Regional:
    - maior salto de indice da regional no mes (vs mes anterior), por marca
    - regional com mais lojas que subiram de Nivel no mes, por marca
    """
    # indice medio do mes anterior por email (mesma logica de indices_por_grupo, mas so pro delta)
    rk_prev=rk[rk["competencia"]==prev_ts][["bkn","indice_eficiencia_mes"]].dropna()
    dp2=dp[dp["Dispara Dashboard Loja"]=="SIM"][["BKN","Email Gerente Regional"]].rename(
        columns={"BKN":"bkn","Email Gerente Regional":"email"})
    mp=rk_prev.merge(dp2,on="bkn")
    idx_prev=mp.groupby("email")["indice_eficiencia_mes"].mean().to_dict()
    salto_por_marca={}  # marca -> email vencedor
    por_marca={}
    for email,marca in peers_marca.items():
        por_marca.setdefault(marca,[]).append(email)
    for marca,emails in por_marca.items():
        deltas={e:(idx_prev[e]-idx_reg[e]) for e in emails if e in idx_prev and e in idx_reg}
        if deltas:
            venc=max(deltas,key=deltas.get)  # maior queda = maior melhora
            if deltas[venc]>0: salto_por_marca[marca]=venc
    # lojas que subiram de Nivel, contadas por regional
    dpn=dp[dp["Dispara Dashboard Loja"]=="SIM"][["BKN","Email Gerente Regional"]].rename(
        columns={"BKN":"bkn","Email Gerente Regional":"email"})
    dpn["bkn"]=dpn["bkn"].astype(str)
    subiu_conta={}  # email -> contagem de lojas que subiram de nivel
    for _,row in dpn.iterrows():
        bkn=row["bkn"]; email=row["email"]
        fx_atual=band_loja.get(bkn); fx_ant=band_loja_prev.get(bkn)
        if fx_atual is None or fx_ant is None or fx_atual==fx_ant: continue
        try:
            if BANDEIRAS_8.index(fx_atual)<BANDEIRAS_8.index(fx_ant):
                subiu_conta[email]=subiu_conta.get(email,0)+1
        except ValueError:
            continue
    mais_lojas_subiram_por_marca={}
    for marca,emails in por_marca.items():
        contagens={e:subiu_conta.get(e,0) for e in emails}
        if contagens:
            venc=max(contagens,key=contagens.get)
            if contagens[venc]>0: mais_lojas_subiram_por_marca[marca]=venc
    return {"salto_indice_por_marca":salto_por_marca,
            "mais_lojas_subiram_por_marca":mais_lojas_subiram_por_marca}

def frases_ego_loja(bkn, marca, marca_cluster_tupla, ego_cluster, ego_marca, ego_melhorou, nomes):
    """Frases de ego para o dashboard de Loja (broadcast: todos veem, texto muda pra
    quem venceu vs quem nao venceu). Criterios comparativos = frase dupla.
    marca_cluster_tupla: (marca,cluster) da propria loja, como retornado por
    bandeiras_por_cluster/cluster_loja."""
    bkn=str(bkn); frases=[]
    # melhor do proprio Nivel/cluster
    venc_cluster=ego_cluster.get(marca_cluster_tupla)
    if venc_cluster:
        nome_venc=nomes.get(venc_cluster, venc_cluster)
        if bkn==venc_cluster:
            frases.append(f"Parabens! Sua loja e a mais eficiente do seu Nivel neste mes.")
        else:
            frases.append(f"A loja {nome_venc} e a mais eficiente do seu Nivel neste mes. Continue buscando esse resultado!")
    # melhor da marca no Brasil
    venc_marca=ego_marca.get(marca)
    if venc_marca and venc_marca!=venc_cluster:
        nome_venc=nomes.get(venc_marca, venc_marca)
        if bkn==venc_marca:
            frases.append("Parabens! Sua loja e a mais eficiente do Brasil na sua marca este mes.")
        else:
            frases.append(f"A loja {nome_venc} e a mais eficiente do Brasil na sua marca este mes.")
    # loja que mais melhorou o indice (criterio comparativo -> frase dupla tambem)
    venc_melhorou=ego_melhorou.get(marca)
    if venc_melhorou:
        nome_venc=nomes.get(venc_melhorou, venc_melhorou)
        if bkn==venc_melhorou:
            frases.append("Parabens! Sua loja foi a que mais melhorou o indice de eficiencia este mes.")
        else:
            frases.append(f"A loja {nome_venc} foi a que mais melhorou o indice de eficiencia este mes.")
    return frases

def bloco_ego_loja(frases):
    if not frases: return ""
    linhas="".join(f'<div style="margin-top:4px;">&#127942; {f}</div>' for f in frases)
    return f"""<div style="margin-top:12px;background:{COR['cinza_bg']};border-radius:10px;padding:12px 16px;">
      <div style="font-size:13px;color:{COR['texto']};">{linhas}</div></div>"""

def frases_ego_coordenador(email, marca, podio_por_marca, salto_nivel):
    """Frases de ego para o dashboard de Coordenador. Podio (comparativo, marca) = frase
    dupla. Recorde proprio e salto de Nivel proprio sao individuais: so aparecem para
    quem conquistou, sem frase generica para o resto."""
    frases=[]
    podio=podio_por_marca.get(marca,[])
    if email in podio:
        pos=podio.index(email)+1
        ord_txt={1:"1o",2:"2o",3:"3o"}[pos]
        frases.append(f"Parabens! Voce e o {ord_txt} coordenador mais eficiente da sua marca este mes.")
    elif podio:
        nome_1=nome_de_email(podio[0])
        frases.append(f"O coordenador {nome_1} tem a carteira mais eficiente da sua marca este mes.")
    # salto de Nivel (individual, so quem subiu ve)
    if email in salto_nivel:
        fx_ant,fx_atual=salto_nivel[email]
        frases.append(f"Parabens! Suas lojas subiram de {fx_ant} para {fx_atual} este mes.")
    return frases

def frase_recorde_proprio_coordenador(email, indice_12m_atual, historico_12m_indices):
    """Individual: so aparece se o indice medio 12m atual e o melhor dos ultimos 12
    meses (registro proprio). historico_12m_indices: lista de indices medios mensais
    da propria carteira nos ultimos 12 meses (menor=melhor)."""
    if not historico_12m_indices or indice_12m_atual is None: return None
    if indice_12m_atual<=min(historico_12m_indices):
        return "Parabens! Este e o melhor indice medio da sua carteira nos ultimos 12 meses."
    return None

def frases_ego_regional(email, marca, salto_indice_por_marca, mais_lojas_por_marca):
    """Frases de ego para o dashboard de Gerente Regional. 2 criterios comparativos,
    cada um com frase dupla independente (uma regional pode vencer em um, nos dois,
    ou em nenhum)."""
    frases=[]
    venc_salto=salto_indice_por_marca.get(marca)
    if venc_salto:
        if email==venc_salto:
            frases.append("Parabens! Sua regional teve o maior salto de indice de eficiencia da marca este mes.")
        else:
            nome_venc=nome_de_email(venc_salto)
            frases.append(f"O gerente {nome_venc} teve o maior salto de indice de eficiencia da marca este mes.")
    venc_lojas=mais_lojas_por_marca.get(marca)
    if venc_lojas:
        if email==venc_lojas:
            frases.append("Parabens! Sua regional teve mais lojas subindo de Nivel do que qualquer outra da marca este mes.")
        else:
            nome_venc=nome_de_email(venc_lojas)
            frases.append(f"O gerente {nome_venc} teve mais lojas subindo de Nivel do que qualquer outro da marca este mes.")
    return frases

def bloco_ego_grupo(frases):
    if not frases: return ""
    linhas="".join(f'<div style="margin-top:4px;">&#127942; {f}</div>' for f in frases)
    return f"""<div style="margin-top:12px;background:{COR['cinza_bg']};border-radius:10px;padding:12px 16px;">
      <div style="font-size:13px;color:{COR['texto']};">{linhas}</div></div>"""
...
def montar_grupo(email, col_email, ref_ts, flr, rk, dp, indices_todos_grupos, front_grupos, p50_idx,
                 ...
                 cluster_por_bkn=None, fronteiras_por_cluster=None, ego_grupo=None):
    ...
    frases_ego=[]
    eg=ego_grupo or {}
    if col_email=="Email Coordenador":
        frases_ego+=frases_ego_coordenador(email, marca_grupo, eg.get("podio_por_marca",{}),
                                           eg.get("salto_nivel",{}))
        rec=frase_recorde_proprio_coordenador(email, indice_12m,
                                              [v for _,v in serie_idx_grupo])
        if rec: frases_ego.append(rec)
    elif col_email=="Email Gerente Regional":
        frases_ego+=frases_ego_regional(email, marca_grupo, eg.get("salto_indice_por_marca",{}),
                                        eg.get("mais_lojas_subiram_por_marca",{}))
    bloco_ego=bloco_ego_grupo(frases_ego)
...
def main():
    ...
    # ---- Ego / autoimagem (v14): pre-calculo unico, reaproveitado por todas as lojas/grupos ----
    ego_lojas={"melhor_cluster":{}, "melhor_marca":{}, "mais_melhorou":{}, "nomes":{}}
    _egl=ranking_ego_lojas(ref_ts, flr, rk, cluster_loja, band_loja)
    ego_lojas["melhor_cluster"]=_egl["melhor_cluster"]
    ego_lojas["melhor_marca"]=_egl["melhor_marca"]
    ego_lojas["nomes"]=_egl["nomes"]
    ego_lojas["mais_melhorou"]=loja_que_mais_melhorou(ref_ts, prev_ts, flr, rk)
    peers_marca_coord={e:p["marca"] for e,p in front_coord.get("peers",{}).items()}
    ego_coord=ranking_ego_coordenadores(idx_coord, front_coord, peers_marca_coord)
    peers_marca_reg={e:p["marca"] for e,p in front_reg.get("peers",{}).items()}
    ego_reg=ranking_ego_regionais(idx_reg, front_reg, peers_marca_reg, dp, ref_ts, prev_ts, rk,
                                  band_loja, band_loja_prev)
    ...
    if MODO_VALIDACAO:
        ...
        for bkn in elegiveis[:QTD_EXEMPLOS]:
            d=dados_loja(bkn,ref_ts,prev_ts,flr,cmp,rk,hist,band_loja,fronteiras_cluster,band_loja_prev,medianas,pools_comp,faixas_loja,cluster_loja,ego_lojas=ego_lojas)
            if not d: continue
        ...
        for email in coords:
            d=montar_grupo(email,"Email Coordenador",ref_ts,flr,rk,dp,idx_coord,front_coord,p50_idx,medianas,hist,setores_indice,cluster_loja,fronteiras_cluster,ego_grupo=ego_coord)
        ...
        for email in regs:
            d=montar_grupo(email,"Email Gerente Regional",ref_ts,flr,rk,dp,idx_reg,front_reg,p50_idx,medianas,hist,setores_indice,cluster_loja,fronteiras_cluster,ego_grupo=ego_reg)
